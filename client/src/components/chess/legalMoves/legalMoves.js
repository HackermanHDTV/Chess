...
const isSameColor = (piece) => piece.isLowerCase() === isWhite
const onBoard = (i, j) => 0 <= i && i < 8 && 0 <= j && j < 8
const baseRank = (i) => (isWhite ? i === 6 : i === 1)
const getCoordinates = (notation) => {
  if (notation === '-') return null
  return [8 - parseInt(notation[1]), notation.charCodeAt(0) - 97]
}

export default function formulateMoves(newFEN) {
  const values = newFEN.split(' ')
...
function pawnMoves(i, j, enPassantSquare) {
  ...
  for (let k = 1; k <= 2; ++k) {
    ...
    movesAvaiable.push(
      new Move(
        ...
        false, // isCapture
        false, // isCastle
        baseRank(i) && k === 2 // isEnPassantSquare
      )
    )
  }
  ...
  for (let k = -1; k <= 1; k += 2) {
    if (
      !isSameColor(i + moveDirection, j + k) ||
      (enPassantSquare &&
        i === enPassantSquare[0] &&
