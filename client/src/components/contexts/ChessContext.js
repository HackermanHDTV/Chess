...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'highlight': {
      return {
        ...state,
        highlightedElements: [...state.highlightedElements, action.payload.id],
      }
    }
    case 'unhighlight': {
      return {
    ...
    case 'move': {
      if (state.emitMove) {
        state.emitMove(action.payload.moveObj)
      }
      ...
      const newFifty =
        !action.payload.moveObj.isCapture && !action.payload.moveObj.isPawnMove
